...
def edit_targets_on_kill(user, targets, settings):
    if settings['passon'] == "pass":
        newWord = targets[user]['word']
    else:
        newWord = get_word(settings)
    targets[targets[user]['assassin']]['word'], targets[user]['word'] = newWord, targets[targets[user]['assassin']]['word'] #swaps words with the assassin
